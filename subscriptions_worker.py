...
def create_download_archive():
    ...
    while not result_archive:
        try:
            ...
            mysql_cursor.execute("SELECT videos.site AS 'site', videos.url AS 'url' FROM videos;")
...
def get_new_channel_videos_from_youtube(channel, ignore_errors):
...
def get_new_playlist_videos_from_youtube(playlist, ignore_errors, counter):
...
def add_video(video, channel_site, channel_id, playlist_id, download):
    ...
    try:
        ...
        try:
            ...
            info_json = get_video_details(video_id)
...
def download_video(video_site, video_id, channel_name, playlist_name):
    # Set the full output path
    full_path = os.path.join(f'{channel_name} - {playlist_name}',
                             f'Season %(release_date>%Y,upload_date>%Y)s [{channel_name}]',
                             f'{channel_name} - {playlist_name} - S%(release_date>%Y,upload_date>%Y)sE%(release_date>%j,upload_date>%j)s - %(title)s.%(ext)s')
    print(f'Path for video "{video_site} {video_id}": {full_path}')


# TODO: rework and remove OLD Main
                ...
                if missing_video_count_channel > 0:
                    ...
                    for current_playlist in channel_playlists:
                        ...
                        if not missing_videos_playlist:
                            print(f'{datetime.now()} {Fore.RED}CRITICAL ERROR{Style.RESET_ALL} while processing playlist "'
                                  f'{current_playlist_name}" ({current_playlist_site} {current_playlist_id})')
